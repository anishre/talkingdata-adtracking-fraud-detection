...
def main():
    ...
    model = xgb.XGBClassifier(
        max_depth=3, n_estimators=600, learning_rate=0.05)

    # fit the model
    #chunksize = 40 000 000
    chunksize = 1000000
    chunk_idx = 0
    chunk_max = 1
    for chunk in pd.read_csv('competition_files/train_sample.csv', chunksize=chunksize):
        # Extracting new features
        chunk['hour'] = pd.to_datetime(
            chunk.click_time).dt.hour.astype('uint8')
        chunk['day'] = pd.to_datetime(chunk.click_time).dt.day.astype('uint8')
        train_X = chunk.as_matrix(
            columns=['ip', 'app', 'device', 'os', 'channel', 'hour', 'day'])
    ...
    # read test data set
    test = pd.read_csv('competition_files/test.csv')
    ...
    test_X = test.as_matrix(
        columns=['ip', 'app', 'device', 'os', 'channel', 'hour', 'day'])
    start_time = print_duration(
        start_time, "Finished training, start prediction")
    # predict the propabilities for binary classes
    pred = model.predict_proba(test_X)

    start_time = print_duration(
        start_time, "Finished prediction, start store results")
    submission = pd.read_csv("competition_files/sample_submission.csv")
    submission['is_attributed'] = pred[:, 1]
    submission.to_csv("output_files/submission-xgboost-wolfgang.csv", index=False)
